...
const page = (props: Props) => {
  ...
  const queryRag = async (query: string) => {
    try {
      const api_url = `https://8a71-223-188-224-219.ngrok-free.app/query-rag/${query}`
      ...
      if (response.ok) {
        ...
        console.log(jsonData.message.response)
      }
  ...
  useEffect(() => {
    ...
    const fetchGraphData = async (stock_name) => {
      const { current_date, prev_year_date } = getCurrentFormattedDate()
      const interval = "1mo"

      try {
        const api_url = `http://127.0.0.1:8000/fetch-ticker-data/${stock_name}/${prev_year_date}/${current_date}/${interval}`
    ...
    const fetchPredictions = async (stock_name)  => {

      const {current_date, prev_year_date}  = getCurrentFormattedDate()
      const interval = "1d"

      try{
        const api_url = `http://127.0.0.1:8000/predict-prices/${stock_name}/${prev_year_date}/${current_date}/${interval}`
  ...
  const stock_info = {
    "HBIO": {
        ...
        about: 'Harvard Bioscience, Inc. is a global developer and manufacturer of life science tools and systems for research and drug discovery. It operates within the biotechnology sector, providing high-quality laboratory instruments and consumables to academic, research, and pharmaceutical customers​​',
        gross_margin: '58.6%',
    ...
    about: "Nathan's Famous, Inc. operates in the food industry, best known for its brand of hot dogs and associated products. It engages in the operation of restaurants, franchising, and licensing its famous brand for various food products​.",
    growth_margin:"30.5%",
    net_profit:"15.2%",
    eps:"$3.45",
    roe:"25.8%",
    ...
    "MYFW": {
      ...
      about: "First Western Financial, Inc. is a financial services company offering wealth advisory, private banking, mortgage lending, and asset management services. It serves high-net-worth individuals, families, and businesses with tailored financial solutions​",
      growth_margin:"none",
      net_profit:"22.5%",
      eps:"-$1.85",
      roe:"12.3%",
    ...
    "IBEX": {
      name:"IBEX Limited",
      about: "IBEX Limited is a leading global provider of outsourced customer engagement services, offering solutions in customer care, technical support, and sales for various industries. The company delivers business process outsourcing (BPO) services to improve client relationships and operational efficiencies​​",
      growth_margin:"35.8%",
      net_profit:"7.5%",
      eps:"-$1.25",
      roe:"14.2%",
